                ...
                for (int y = 0; y < _maze_h; ++y) {
                    ...
                    for (int p = 0; p < _cell_w; ++p) {
                        if (cell & cell_attrib::path_s) {
                            draw(x * cell_plus_wall_w + p + pixel_offset_x, y * cell_plus_wall_w + _cell_w + pixel_offset_y, pixel_type::solid, color_t::white);
                        }
                        if (cell & cell_attrib::path_e) {
                            draw(x * cell_plus_wall_w + _cell_w + pixel_offset_x, y * cell_plus_wall_w + p + pixel_offset_y, pixel_type::solid, color_t::white);
                        }
                    }
                }
            }
